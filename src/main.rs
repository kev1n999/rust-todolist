...
impl TodoList {
    ...
    fn get_task_by_filter(&self, filter: Filter) -> Option<Vec<&Task>> {
        ...
        let tasks: Vec<&Task> = match filter {
            ...
            Filter::Priority(priority) => self
                .tasks
                .iter()
                .filter(|task| task.priority == priority)
                .collect(),

            Filter::Id(id) => self.tasks.iter().filter(|task| task.id == id).collect(),
        };
...
fn get_priority_and_status(priority: &str, status: &str) -> Option<(Priority, Status)> {
...
fn get_atributtes(
    ...
    io::stdin().read_line(name).expect("an error ocurred");
    ...
    io::stdin().read_line(status).expect("an error ocurred");

    println!("[Task priority][Low, Medium, High] Type the task priority: ");
    io::stdin().read_line(priority).expect("an error ocurred");

    println!("[Task id] Type the task id: ");
    io::stdin().read_line(id).expect("an error ocurred");
}
        ...
        match command.trim().chars().next() {
            ...
            Some('3') => {
                ...
                match filter_option.trim().to_lowercase().as_str() {
                    ...
                    "id" => println!("{:?}", todo.get_task_by_filter(Filter::Id(filter_content))),
